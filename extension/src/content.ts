...
type PendingSubmission = { title: string; url: string; startedAt: number; activeMinutes: number; code: string };
type StoredState = {
  appUrl: string;
  token?: string;
...
root.innerHTML = `<style>
  ...
  .success{width:250px;min-height:190px;padding:27px;display:grid;place-items:center;align-content:center;border-radius:20px;text-align:center}.success-ring{width:68px;height:68px;display:grid;place-items:center;border-radius:50%;background:#c8f25a;color:#17200e;box-shadow:0 0 0 9px rgba(200,242,90,.12);animation:success-pop .45s cubic-bezier(.2,.9,.3,1.25)}.success-ring svg{width:36px;height:36px;fill:none;stroke:currentColor;stroke-width:2.6;stroke-linecap:round;stroke-linejoin:round}.success h2{margin:20px 0 3px;font-size:20px}.success p{margin:0;color:#a8ae9f;font-size:11px}.success-again{display:flex;align-items:center;justify-content:center;gap:7px;margin-top:16px;min-height:34px;padding:0 13px;border:1px solid #53613a;border-radius:10px;background:#2d3824;color:#dfff9d;font-size:11px;font-weight:800;cursor:pointer;transition:.16s}.success-again:hover{background:#3a4a2d;border-color:#819b52}.success-again svg{width:14px;height:14px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}@keyframes success-pop{0%{transform:scale(.3);opacity:0}100%{transform:scale(1);opacity:1}}
...
<div class="tracker" data-placement="bottom-left">
  ...
  <section class="surface success hidden" data-view="success"><div class="success-ring"><svg viewBox="0 0 24 24"><path d="m5 12.5 4.2 4.2L19 7"/></svg></div><h2>Good job.</h2><p>Session saved. Keep the flow.</p><button class="success-again" aria-label="Start another submission" title="Start another submission"><svg viewBox="0 0 24 24"><path d="M20 11a8.1 8.1 0 0 0-14.9-4L3 9m0 0V4m0 5h5M4 13a8.1 8.1 0 0 0 14.9 4L21 15m0 0v5m0-5h-5"/></svg><span>Try another</span></button></section>
</div>`;
...
const problemTitle = () => document.title.replace(/\s*-\s*LeetCode.*$/i, "").trim() || location.pathname.split("/").filter(Boolean).at(-1)?.replaceAll("-", " ") || "LeetCode problem";
...
function render() {
  ...
  $$<HTMLElement>("[data-view]").forEach((view) => view.classList.toggle("hidden", view.dataset.view !== mode));
  ...
  $$<HTMLElement>(".dot").forEach((dot) => dot.classList.toggle("live", timer?.status === "running"));
...
async function playSuccess() {
  try {
    ...
    [523.25, 659.25, 783.99].forEach((frequency, index) => {
      ...
      gain.gain.linearRampToValueAtTime(.07, now + index * .08 + .015);
      gain.gain.exponentialRampToValueAtTime(.001, now + index * .08 + .22);
      oscillator.connect(gain).connect(context.destination);
      oscillator.start(now + index * .08);
      oscillator.stop(now + index * .08 + .23);
    });
  } catch { /* Sound is a bonus; saving must never depend on it. */ }
}

function selectedValue(group: string) {
  return $<HTMLButtonElement>(`[data-group="${group}"] button.selected`).dataset.value ?? "";
}
...
function collectFields() {
  return {
    ...
    template: $<HTMLSelectElement>("[name=template]").value,
    topics: $<HTMLInputElement>("[name=topics]").value.split(",").map((value) => value.trim()).filter(Boolean),
    approach: $<HTMLTextAreaElement>("[name=approach]").value.trim(),
    blocker: $<HTMLTextAreaElement>("[name=blocker]").value.trim(),
    hint: $<HTMLTextAreaElement>("[name=hint]").value.trim(),
    notes: $<HTMLTextAreaElement>("[name=notes]").value.trim(),
    needsVisual: $<HTMLInputElement>("[name=needsVisual]").checked,
  };
}
...
$$<HTMLButtonElement>("[data-group] button").forEach((button) => button.addEventListener("click", () => {
