...
(() => {
  ...
  var HOST_ID = "unfuckdsa-problemset-revisions";
  var revisions = [];
  var errorMessage = "";
  var loading = true;
  ...
  function render() {
    ...
    const queue = shadow.querySelector(".queue");
    ...
    if (loading || errorMessage || !revisions.length) {
      ...
      message.textContent = loading ? "Loading your revision queue\u2026" : errorMessage || "Revision queue clear for today.";
      rows.append(message);
      return;
    }
    revisions.forEach((revision, index) => {
      ...
      row.querySelector(".copy").href = revision.url;
      row.querySelector(".title").textContent = revision.title;
      row.querySelector(".meta").textContent = revision.topics.slice(0, 2).join(" \xB7 ") || revision.reason[0] || "Due for recall";
      row.querySelector(".done").addEventListener(
        "click",
        (event) => {
          event.preventDefault();
          event.stopPropagation();
          void complete(revision, event.currentTarget);
        }
      );
      rows.append(row);
    });
  }
  async function complete(revision, button) {
