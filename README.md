...
The extension owns timer and session state and survives refreshes/browser restarts. Submitted sessions are kept in extension storage and imported by the dashboard when it opens. Once paired, the LeetCode Problemset page also shows the day’s revision queue above the native list; its check action records a `Good` review and advances repetition. It captures the user-authored editor buffer only when **End** is explicitly pressed. It does not scrape profiles, copy problem statements, inspect submission traffic, or infer acceptance.
